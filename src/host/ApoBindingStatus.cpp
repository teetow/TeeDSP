...
namespace {
// deviceId looks like "{0.0.0.00000000}.{<endpoint-guid>}" — the registry key
// is filed under just the trailing guid.
QString fxPropertiesKeyPath(const QString &renderDeviceId)
{
    const int dot = renderDeviceId.lastIndexOf(QLatin1Char('.'));
    const QString guid = (dot >= 0) ? renderDeviceId.mid(dot + 1) : renderDeviceId;
    return QStringLiteral("HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion"
                          "\\MMDevices\\Audio\\Render\\") + guid + QStringLiteral("\\FxProperties");
}
} // namespace
...
ApoBindingInfo queryApoBinding(const QString &renderDeviceId)
{
    ...
    QSettings fx(fxPropertiesKeyPath(renderDeviceId), QSettings::NativeFormat);
...
ApoBindingRaw queryApoBindingRaw(const QString &renderDeviceId)
{
    ...
    raw.sfxClsid = fx.value(pkeyBase + QStringLiteral(",5")).toString();
    raw.mfxClsid = fx.value(pkeyBase + QStringLiteral(",6")).toString();
    raw.compositeMfxClsid = fx.value(pkeyBase + QStringLiteral(",14")).toString();
