...
bool ApoSharedClient::readStatus(ApoStatus &out) const
{
    ...
    out.bytesPerFrame = m_shm->bytesPerFrame;
    out.uiAlive    = m_shm->uiAlive;
    std::atomic_ref<uint64_t> pc(m_shm->processCalls);
    out.processCalls = pc.load(std::memory_order_relaxed);
    std::atomic_ref<uint64_t> fp(m_shm->framesProcessed);
    out.framesProcessed = fp.load(std::memory_order_relaxed);
    std::atomic_ref<uint64_t> lbf(m_shm->lastBufferFlags);
    out.lastBufferFlags = lbf.load(std::memory_order_relaxed);
    std::atomic_ref<uint32_t> pg(m_shm->paramGen);
    out.paramGen = pg.load(std::memory_order_relaxed);
    std::atomic_ref<uint32_t> ag(m_shm->appliedGen);
    out.appliedGen = ag.load(std::memory_order_relaxed);
    std::atomic_ref<uint32_t> mo(m_shm->meterOwner);
    out.meterOwner = mo.load(std::memory_order_relaxed);
    std::atomic_ref<uint64_t> hb(m_shm->uiHeartbeat);
    out.uiHeartbeat = hb.load(std::memory_order_relaxed);
